...
def compute_affine_registration(fixed_volume_path: str,
    ...
    print(f"Original volumes - Fixed: {fixed_volume.shape}, Moving: {moving_volume.shape}")
    ...
    print(f"Downsampled volumes - Fixed: {fixed_downsampled.shape}, Moving: {moving_downsampled.shape}")
    ...
    affine_matrix = alignment_pipeline(
        fixed_downsampled, moving_downsampled,
...
def compute_deformation_field_registration(fixed_zarr_path: str,
                                         ...
                                         cluster_config: Optional[Dict] = None) -> str:
    """
    ...
    affine_aligned_path = os.path.join(output_directory, f"{output_name}_affine_aligned.zarr")
    ...
    deformation_field_path = os.path.join(output_directory, f"{output_name}_deformation_field.zarr")
    
    distributed_piecewise_alignment_pipeline(
        fixed_volume, affine_aligned_volume,
        voxel_spacing, voxel_spacing,
        deformation_steps,
        blocksize=block_size,
        overlap=0.3,
    ...
    final_aligned_path = os.path.join(output_directory, f"{output_name}_final_aligned.zarr")
...
def distributed_nuclei_segmentation(input_zarr_path: str,
                                   output_zarr_path: str,
                                   model_type: str = "cpsam",
                                   block_size: Tuple[int, int, int] = (500, 1024, 1024),
                                   preprocessing_sigma: float = 2.0,
                                   cluster_config: Optional[Dict] = None,
                                   temporary_directory: Optional[str] = None) -> Tuple[Any, List]:
    """
...
def apply_deformation_to_channels(reference_zarr_path: str,
    ...
    for i, channel_path in enumerate(channel_zarr_paths):
        ...
        distributed_apply_transform(
            reference_volume, channel_volume,
            voxel_spacing, voxel_spacing,
            transform_list=[deformation_field],
            blocksize=block_size,
...
def create_registration_summary(fixed_path: str,
                               moving_path: str,
                               affine_matrix_path: str,
                               deformation_field_path: str,
                               final_aligned_path: str,
                               output_summary_path: str) -> Dict[str, Any]:
    """
