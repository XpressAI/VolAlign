...
def create_dapi_channel(segmentation_mask, noise_level=0.1):
    ...
    logger.info(f"DAPI channel: min={dapi.min()}, max={dapi.max()}, mean={dapi.mean():.1f}")
    return dapi


def create_epitope_channel(segmentation_mask, positive_probability=0.3, signal_strength=0.8):
    """
    ...
    for label_id in range(1, max_label + 1):
        if positive_nuclei[label_id - 1]:  # -1 because array is 0-indexed
            ...
            if np.any(nucleus_mask):
                ...
                epitope[dilated_mask] = base_intensity * intensity_variation[dilated_mask]
                positive_count += 1
    ...
    logger.info(f"Epitope channel: {positive_count}/{max_label} positive nuclei, "
...
def save_zarr_dataset(data, output_path, array_key=None, compression='zstd'):
    ...
    if array_key:
        ...
        store.create_dataset(
            array_key,
            data=data,
            chunks=(min(10, data.shape[0]), min(64, data.shape[1]), min(64, data.shape[2])),
...
def main():
    parser = argparse.ArgumentParser(description="Generate synthetic test data for Nuclei Viewer")
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Number of nuclei to generate"
    )
    parser.add_argument(
        "--epitope-channels",
        type=str,
    ...
    if args.use_groups:
        ...
        save_zarr_dataset(segmentation, output_dir / "segmentation_labels.zarr", "labels")
    ...
    summary = {
        ...
        "files": {
            ...
            "epitope_channels": {name: f"{name}_channel.zarr" for name in args.epitope_channels}
        }
    }
