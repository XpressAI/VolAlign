...
class ProcessingConfig(BaseModel):
    ...
    def validate_percentile_range(cls, v):
        if len(v) != 2 or v[0] >= v[1] or v[0] < 0 or v[1] > 100:
            raise ValueError('percentile_range must be [min, max] with 0 <= min < max <= 100')
        return v
...
class UIConfig(BaseModel):
    ...
    color_palette: List[str] = [
        ...
        "#00ffff", "#ff8000", "#8000ff", "#80ff00", "#0080ff"
    ]
...
class Settings(BaseSettings):
    ...
    config_file: str = Field(default="config/default_config.yaml", env="NUCLEI_VIEWER_CONFIG")
