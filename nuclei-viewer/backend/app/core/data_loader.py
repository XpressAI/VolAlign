...
class DatasetInfo:
    ...
    def __init__(self, name: str, path: Path, shape: Tuple[int, ...], dtype: str, array_key: Optional[str] = None):
        self.name = name
        self.path = path
        self.shape = shape
        self.dtype = dtype
    ...
    def __repr__(self) -> str:
        return f"DatasetInfo(name='{self.name}', shape={self.shape}, dtype='{self.dtype}')"


class NucleusInfo:
    """Information about a segmented nucleus."""
    
    def __init__(self, label: int, bbox: Tuple[int, ...], area: int, centroid: Tuple[float, ...]):
        self.label = label
...
class DataLoader:
    ...
    def discover_datasets(self) -> Dict[str, List[Path]]:
        ...
        logger.info(f"Discovered datasets: {[(k, len(v)) for k, v in discovered.items()]}")
        return discovered
    ...
    def load_dapi_channel(self, file_path: Optional[Path] = None, array_key: Optional[str] = None) -> DatasetInfo:
    ...
    def load_epitope_channel(self, name: str, file_path: Path, array_key: Optional[str] = None) -> DatasetInfo:
    ...
    def load_epitope_channels(self, file_paths: Optional[Dict[str, Path]] = None) -> Dict[str, DatasetInfo]:
        ...
        for epitope_config in self.config.data.epitope_channels:
            if epitope_config.name not in file_paths:
                logger.warning(f"No file found for epitope channel: {epitope_config.name}")
                continue
            ...
            try:
                ...
                logger.error(f"Failed to load epitope channel {epitope_config.name}: {e}")
    ...
    def get_nuclei_page(self, page: int, page_size: Optional[int] = None) -> List[NucleusInfo]:
        """
...
def load_zarr_as_dask(zarr_path: Union[str, Path], array_key: Optional[str] = None) -> da.Array:
    """
    ...
    try:
        ...
        if hasattr(zarr_store, 'shape'):
            ...
            if array_key is not None:
                logger.warning(f"array_key '{array_key}' specified but {zarr_path} is a direct zarr array")
            dask_array = da.from_zarr(zarr_store)
        else:
            # This is a zarr group with keys
            if array_key is None:
                available_keys = list(zarr_store.keys())
                raise KeyError(f"array_key required for zarr group. Available keys: {available_keys}")
            ...
            try:
                ...
                available_keys = list(zarr_store.keys())
                raise KeyError(f"Array '{array_key}' not found in zarr store. Available keys: {available_keys}")
        
        logger.debug(f"Loaded zarr array: shape={dask_array.shape}, dtype={dask_array.dtype}, chunks={dask_array.chunks}")
        return dask_array
...
def validate_shape_compatibility(datasets: List[DatasetInfo]) -> Tuple[int, ...]:
    ...
    if needs_cropping:
        logger.warning(f"Shape mismatch detected. Will crop to common shape: {min_shape}")
