...
async def discover_datasets(
    base_path: Optional[str] = Query(None, description="Override base path for discovery"),
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Dataset discovery failed: {str(e)}")
...
async def load_datasets(
    ...
    try:
        ...
        try:
            ...
            segmentation = data_loader.load_segmentation(seg_file, request.segmentation.array_key)
        ...
        try:
            ...
            dapi = data_loader.load_dapi_channel(dapi_file, request.dapi_channel.array_key)
            ...
            for channel_req in request.epitope_channels:
                try:
                    ...
                    dataset = data_loader.load_epitope_channel(
                        channel_req.name,
                        channel_file,
                        channel_req.array_key
                    )
                    epitope_channels[channel_req.name] = dataset
                except Exception as e:
                    logger.error(f"Failed to load epitope channel {channel_req.name}: {e}")
...
async def load_all_datasets(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Auto-loading datasets failed: {str(e)}")
...
async def get_data_status(data_loader: DataLoader = Depends(get_data_loader)):
    ...
    try:
        status = {
            ...
            'total_nuclei': len(data_loader.nuclei) if data_loader._nuclei_loaded else 0
        }
...
async def get_available_channels(data_loader: DataLoader = Depends(get_data_loader)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get channel information: {str(e)}")
