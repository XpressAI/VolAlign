...
from ..core.config import get_config, update_config, save_config, AppConfig, EpitopeChannelConfig
...
async def save_configuration(
    config_path: Optional[str] = Query(None, description="Path to save configuration file")
):
    """
...
async def get_data_paths():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get data paths: {str(e)}")
...
async def update_base_path(base_path: str):
    ...
    try:
        ...
        if not path_obj.exists():
            raise HTTPException(status_code=400, detail=f"Path does not exist: {base_path}")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to update base path: {str(e)}")
...
async def add_epitope_channel(request: EpitopeChannelRequest):
    ...
    try:
        ...
        if request.name in existing_names:
            raise HTTPException(status_code=400, detail=f"Epitope channel '{request.name}' already exists")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to add epitope channel: {str(e)}")
...
async def update_epitope_channel(channel_name: str, request: EpitopeChannelRequest):
    ...
    try:
        ...
        if channel_index is None:
            raise HTTPException(status_code=404, detail=f"Epitope channel '{channel_name}' not found")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to update epitope channel: {str(e)}")
...
async def remove_epitope_channel(channel_name: str):
    ...
    try:
        ...
        epitope_channels = [ch for ch in config.data.epitope_channels if ch.name != channel_name]
        ...
        raise HTTPException(status_code=500, detail=f"Failed to remove epitope channel: {str(e)}")
...
async def get_processing_config():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get processing config: {str(e)}")


@router.put("/processing")
async def update_processing_config(
    min_object_size: Optional[int] = Query(None, ge=1, description="Minimum object size in voxels"),
    pad_xy: Optional[int] = Query(None, ge=0, description="XY padding around objects"),
    max_objects_per_page: Optional[int] = Query(None, ge=1, le=100, description="Max objects per page"),
    ...
    percentile_min: Optional[float] = Query(None, ge=0, le=100, description="Min percentile for contrast"),
    percentile_max: Optional[float] = Query(None, ge=0, le=100, description="Max percentile for contrast")
):
    """
    ...
    try:
        ...
        if percentile_min is not None or percentile_max is not None:
            ...
            if new_min >= new_max:
                raise HTTPException(status_code=400, detail="percentile_min must be less than percentile_max")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to update processing config: {str(e)}")
...
async def get_ui_config():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get UI config: {str(e)}")


@router.put("/ui")
async def update_ui_config(
    default_opacity: Optional[float] = Query(None, ge=0.0, le=1.0, description="Default channel opacity"),
    color_palette: Optional[List[str]] = Query(None, description="Color palette for channels")
):
    """
    ...
    try:
        ...
        if color_palette is not None:
            ...
            for color in color_palette:
                if not (color.startswith('#') and len(color) == 7):
                    raise HTTPException(status_code=400, detail=f"Invalid color format: {color}")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to update UI config: {str(e)}")