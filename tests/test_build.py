    ...
    def test_wheel_build(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                assert (
                    "volalign" in wheel_file.name
                ), f"Wheel name doesn't contain package name: {wheel_file.name}"
    ...
    def test_source_distribution_build(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                assert (
                    result.returncode == 0
                ), f"Source distribution build failed: {result.stderr}"
                ...
                assert (
                    "volalign" in sdist_file.name
                ), f"Sdist name doesn't contain package name: {sdist_file.name}"
    ...
    def test_wheel_installation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                ), f"Wheel build failed: {build_result.stderr}"
                ...
                ), f"Package import failed after wheel installation: {import_result.stderr}"
                ...
                try:
                    ...
                    with zipfile.ZipFile(wheel_file, "r") as zip_file:
                        ...
                        volalign_files = [
                            f
                            for f in file_list
                            if "VolAlign" in f or "volalign" in f.lower()
                        ]
                        assert (
                            len(volalign_files) > 0
                        ), f"VolAlign package files not found in wheel. Files: {file_list[:10]}"

                except Exception as e:
                    # If inspection fails, just check that wheel file exists and has reasonable size
                    assert (
                        wheel_file.stat().st_size > 1000
                    ), f"Wheel file seems too small: {wheel_file.stat().st_size} bytes"
...
class TestBuildRequirements:
    ...
    def test_pyproject_toml_valid(self):
        ...
        assert (
            project["name"] == "VolAlign"
        ), f"Expected project name 'VolAlign', got '{project['name']}'"
    ...
    def test_setup_py_compatibility(self):
        ...
        try:
            os.chdir(package_root)
            result = subprocess.run(
                [sys.executable, "setup.py", "--help"], capture_output=True, text=True
            )
