...
class TestFunctionAvailability:
    ...
    def test_utils_functions_available(self):
        ...
        ), "utils module should have callable functions"
    ...
    def test_pipeline_orchestrator_classes(self):
        ...
        assert hasattr(
            pipeline_orchestrator, "MicroscopyProcessingPipeline"
        ...
        assert isinstance(
            pipeline_class, type
        ), "MicroscopyProcessingPipeline should be a class"

        # Check for classes (adjust based on actual implementation)
        available_classes = [
            name
            for name in dir(pipeline_orchestrator)
    ...
    def test_alignment_tools_functions_available(self):
        ...
        for func_name in expected_functions:
            assert hasattr(
                alignment_tools, func_name
            ), f"Function {func_name} not found in alignment_tools module"
            assert callable(
                getattr(alignment_tools, func_name)
    ...
    def test_distributed_processing_functions_available(self):
        ...
        for func_name in expected_functions:
            assert hasattr(
                distributed_processing, func_name
            ), f"Function {func_name} not found in distributed_processing module"
            assert callable(
...
class TestOptionalDependencies:
    ...
    def test_import_with_missing_optional_deps(self):
        ...
        try:
            from VolAlign import alignment_tools, utils
            ...
            if any(
                core_dep in str(e).lower() for core_dep in ["numpy", "scipy", "zarr"]
            ):
